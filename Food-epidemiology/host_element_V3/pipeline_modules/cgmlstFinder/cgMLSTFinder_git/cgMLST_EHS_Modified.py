        ...
        for entry, d in md5_alleles.items():
            if d["seq"] != "":
                ...
                if set(d["seq"]) == {"a", "t", "c", "g"}:
                    md5_alleles[entry]["md5"] = hashlib.md5(str(d["seq"]).encode("utf-8")).hexdigest()
        ...
        with open(hyp_file, "w") as fh:
            for locus in loci_list:
                ...
                if locus in best_alleles:
                    ...
                    # Return the md5 value of the new allele - Edward (G37543428) 02/02/24
                    description = md5_dict[locus]["md5"]
