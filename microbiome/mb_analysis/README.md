# mb_analysis -- ARAC DADA2 Pipeline for 16S rRNA Sequencing Processing
Welcome to the ARAC DADA2 Pipeline for 16S rRNA sequencing processing. This pipleine takes 16S rRNA illumina pair-end sequencing reads as input, performs quality checks, processes the reads to retain high quality data only and creates aggregated tables for the user for further analysis. The pipeline incorportes modules from the DADA2 R library (https://github.com/benjjneb/dada2) , qiime1 and RDP classifier (https://sourceforge.net/projects/rdp-classifier/files/rdp-classifier/). Species level taxonomic classification is performed with an in-house custom trained RDP classifier (V6-Species) for select species, and any ASVs outside the classifier's scope are characterized using blast against NCBI databases.
